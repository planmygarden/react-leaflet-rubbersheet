...
class MatrixUtil {
  ...
  static basisToPoints(x1, y1, x2, y2, x3, y3, x4, y4) {
    ...
    const v = MatrixUtil.multmv(MatrixUtil.adj(m), [x4, y4, 1]);
  ...
  static general2DProjection(
    ...
    const s = MatrixUtil.basisToPoints(x1s, y1s, x2s, y2s, x3s, y3s, x4s, y4s);
    const d = MatrixUtil.basisToPoints(x1d, y1d, x2d, y2d, x3d, y3d, x4d, y4d);
    const m = MatrixUtil.multmm(d, MatrixUtil.adj(s));
